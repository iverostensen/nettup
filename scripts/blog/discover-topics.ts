...
export async function selectTopic(): Promise<Topic> {
  const existingSlugs = readExistingSlugs();

  // Select cluster by remaining capacity (weighted by priority)
  ...
  const selectedCluster = clusterWithCapacity.cluster;

  // Ask Claude for a unique topic angle (up to 3 attempts)
